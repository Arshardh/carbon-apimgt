...
public class TiersApi  {
   ...
   private final TiersApiService delegate = TiersApiServiceFactory.getTiersApi();
    ...
    @Consumes({ "application/json" })
    @Produces({ "application/json" })
    @io.swagger.annotations.ApiOperation(value = "Add a new Tier", notes = "This operation can be used to create a new throttling tier. The only supported tier level is `api` tiers.\n`POST https://127.0.0.1:9443/api/am/publisher/v0.9/tiers/api`\n", response = TierDTO.class)
    @io.swagger.annotations.ApiResponses(value = { 

...
public class TiersApi  {
   ...
   private final TiersApiService delegate = TiersApiServiceFactory.getTiersApi();
    ...
    @Consumes({ "application/json" })
    @Produces({ "application/json" })
    @io.swagger.annotations.ApiOperation(value = "Get available tiers", notes = "This operation can be used to retrieve all the tiers available for the provided tier level. Tier level should be specified as a path parameter and should be one of `api` and `application`.\n\n`X-WSO2-Tenant` header can be used to retrive tiers of a different tenant domain. If not specified, the user's tenant associated with the access token will be used.", response = TierListDTO.class, responseContainer = "List")
    ...
    @Consumes({ "application/json" })
    @Produces({ "application/json" })
    @io.swagger.annotations.ApiOperation(value = "Get tier details", notes = "This operation can be used to retrieve a specific tier available for the provided tier level. Tier level should be specified as a path parameter and should be one of `api` and `application`.\n\n`X-WSO2-Tenant` header can be used to retrive tiers of a different tenant domain. If not specified, the user's tenant associated with the access token will be used.", response = TierDTO.class)
    @io.swagger.annotations.ApiResponses(value = { 
        @io.swagger.annotations.ApiResponse(code = 200, message = "OK.\nTier returned"),

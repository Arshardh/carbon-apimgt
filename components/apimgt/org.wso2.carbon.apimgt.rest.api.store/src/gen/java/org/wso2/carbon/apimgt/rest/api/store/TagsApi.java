...
public class TagsApi  {
   ...
   private final TagsApiService delegate = TagsApiServiceFactory.getTagsApi();
    ...
    @Consumes({ "application/json" })
    @Produces({ "application/json" })
    @io.swagger.annotations.ApiOperation(value = "Retrieve a tag list", notes = "Get a list of tags that are already added to APIs\n\n`X-WSO2-Tenant` header can be used to retrive tags of a different tenant domain. If not specified, the user's tenant associated with the access token will be used.", response = TagListDTO.class)
